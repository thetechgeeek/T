...
export default function InvoiceDetailScreen() {
  ...
  return (
    <Screen safeAreaEdges={['top']} withKeyboard={false}>
      <View style={[styles.header, theme.layout.rowBetween, { borderBottomColor: c.border, borderBottomWidth: 1 }]}>
        <View style={[theme.layout.row, { flex: 1 }]}>
          <ArrowLeft color={c.onBackground} size={24} onPress={() => router.back()} style={{ marginRight: s.md }} />
          <ThemedText variant="h2" style={{ flex: 1 }}>{currentInvoice.invoice_number}</ThemedText>
        </View>
        <Button
          title="Share PDF"
          variant="outline"
      ...
      <ScrollView contentContainerStyle={{ padding: s.lg }}>
          ...
          {currentInvoice.line_items?.map((item, index) => (
            <View key={item.id} style={{ padding: s.md, borderBottomWidth: index === currentInvoice.line_items!.length - 1 ? 0 : 1, borderBottomColor: c.border }}>
              ...
              <View style={[theme.layout.rowBetween, { marginTop: 4 }]}>
                <ThemedText variant="caption" color={c.onSurfaceVariant}>{item.quantity} units @ {formatCurrency(item.rate_per_unit)}</ThemedText>
                <ThemedText weight="semibold">{formatCurrency(item.line_total)}</ThemedText>
              </View>
            </View>
          ))}
        </View>
        <View style={{ marginTop: s.xl, backgroundColor: c.surface, padding: s.md, borderRadius: theme.borderRadius.md, borderWidth: 1, borderColor: c.border }}>
          <View style={theme.layout.rowBetween}>
            ...
            <ThemedText color={c.onSurface}>{formatCurrency(currentInvoice.subtotal)}</ThemedText>
          </View>
          {currentInvoice.is_inter_state ? (
            <View style={theme.layout.rowBetween}>
              <ThemedText color={c.onSurfaceVariant}>IGST</ThemedText>
              <ThemedText color={c.onSurface}>{formatCurrency(currentInvoice.igst_total)}</ThemedText>
            </View>
          ) : (
            <>
              <View style={theme.layout.rowBetween}>
                <ThemedText color={c.onSurfaceVariant}>CGST</ThemedText>
                <ThemedText color={c.onSurface}>{formatCurrency(currentInvoice.cgst_total)}</ThemedText>
              </View>
              <View style={theme.layout.rowBetween}>
                <ThemedText color={c.onSurfaceVariant}>SGST</ThemedText>
                <ThemedText color={c.onSurface}>{formatCurrency(currentInvoice.sgst_total)}</ThemedText>
              </View>
            </>
          )}
