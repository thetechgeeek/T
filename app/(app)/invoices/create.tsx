          ...
          <View>
            ...
            {isAddingItem && (
              <View style={{ marginTop: s.xl, padding: s.lg, backgroundColor: c.surfaceVariant + '40', borderRadius: theme.borderRadius.md, borderWidth: 1, borderColor: c.border }}>
                <Text style={{ color: c.onSurface, fontWeight: 'bold', marginBottom: s.xs }}>Select from Inventory</Text>
                ...
                {!selectedItem ? (
                  <>
                    ...
                    <ScrollView style={{ maxHeight: 200 }}>
                      {items.filter(i => i.design_name.toLowerCase().includes(searchQuery.toLowerCase())).slice(0, 10).map(item => (
                        <TouchableOpacity 
                          key={item.id} 
