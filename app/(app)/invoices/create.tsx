...
import { View, StyleSheet, ScrollView, TextInput, TouchableOpacity, Alert, Platform, ActivityIndicator } from 'react-native';
  ...
  return (
    <Screen withKeyboard safeAreaEdges={['top', 'bottom']}>
      <View style={[styles.stepperMenu, { borderBottomColor: c.border }]}>
        <ThemedText variant="label" color={step === 1 ? c.primary : c.onSurfaceVariant}>1. Customer</ThemedText>
        <ThemedText variant="label" color={step === 2 ? c.primary : c.onSurfaceVariant}>2. Items</ThemedText>
        <ThemedText variant="label" color={step === 3 ? c.primary : c.onSurfaceVariant}>3. Review</ThemedText>
      </View>

      <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: s.lg }}>
        {step === 1 && (
          <View>
            <ThemedText variant="h3" style={{ marginBottom: s.md }}>Customer Details</ThemedText>
          ...
          <View>
            ...
            {lineItems.length === 0 ? (
              <View style={{ padding: s.xl, alignItems: 'center', backgroundColor: c.surface, borderRadius: theme.borderRadius.md }}>
                <ThemedText variant="caption" color={c.placeholder} align="center">No items added yet.</ThemedText>
              </View>
            ) : (
              lineItems.map((item, index) => (
                <View key={index} style={{ padding: s.md, marginBottom: s.sm, backgroundColor: c.surface, borderRadius: theme.borderRadius.sm, borderWidth: 1, borderColor: c.border }}>
                  ...
                  <View style={theme.layout.rowBetween}>
                    <ThemedText variant="caption" color={c.onSurfaceVariant}>{item.quantity} units @ ₹{item.rate_per_unit.toFixed(2)}</ThemedText>
                    <ThemedText weight="bold" color={c.primary}>₹{(item.quantity * item.rate_per_unit).toFixed(2)}</ThemedText>
                  </View>
                  {!!item.discount && item.discount > 0 && (
                     <ThemedText variant="caption" color={c.error}>Discount: ₹{item.discount.toFixed(2)}</ThemedText>
                  )}
                  <TouchableOpacity onPress={() => {
                    ...
                    <ThemedText variant="caption" color={c.error}>Remove</ThemedText>
                  </TouchableOpacity>
                </View>
              ))
            )}
              ...
              <View style={{ marginTop: s.xl, padding: s.lg, backgroundColor: c.surfaceVariant + '40', borderRadius: theme.borderRadius.md, borderWidth: 1, borderColor: c.border }}>
                ...
                {!selectedItem ? (
                  <>
                    ...
                    <ScrollView style={{ maxHeight: 200 }}>
                      {items.length === 0 && !inventoryLoading ? (
                        <ThemedText variant="caption" color={c.placeholder} align="center" style={{ padding: s.md }}>No items found.</ThemedText>
                      ) : (
                        items.map(item => (
                          <TouchableOpacity 
                            key={item.id} 
                          ...
                          >
                            ...
                            <ThemedText variant="caption" color={c.onSurfaceVariant}>Stock: {item.box_count} • Price: ₹{item.selling_price}</ThemedText>
                          </TouchableOpacity>
                        ))
                      )}
                    </ScrollView>
                  ...
                  <View>
                    ...
                    <ThemedText variant="body2" color={c.onSurfaceVariant} style={{ marginBottom: s.md }}>Available: {selectedItem.box_count} units</ThemedText>
...
... (rest is okay)
        ...
        {step === 3 && (
          <View>
            <ThemedText variant="h3" style={{ marginBottom: s.md }}>Review & Payment</ThemedText>
            ...
            <View style={{ padding: s.md, backgroundColor: c.surface, borderRadius: theme.borderRadius.md, borderWidth: 1, borderColor: c.border, marginBottom: s.lg }}>
              ...
              <ThemedText variant="caption" color={c.onSurfaceVariant}>{customer?.phone || 'No phone provided'}</ThemedText>
              
              <View style={{ marginTop: s.md }}>
                {lineItems.map((item, idx) => (
                  <View key={idx} style={[theme.layout.rowBetween, { marginTop: 4 }]}>
                    <ThemedText variant="body2">{item.quantity}x {item.design_name}</ThemedText>
                    <ThemedText variant="body2">₹{(item.quantity * item.rate_per_unit).toFixed(2)}</ThemedText>
                  </View>
                ))}
              </View>
              ...
              <View style={theme.layout.rowBetween}>
                ...
                <ThemedText variant="h3" color={c.primary}>₹{grandTotal.toFixed(2)}</ThemedText>
              </View>
            </View>

            <ThemedText weight="bold" style={{ marginBottom: s.sm }}>Payment Collection</ThemedText>
            ...
            <ThemedText variant="caption" color={c.onSurfaceVariant} style={{ marginBottom: 4 }}>Payment Mode</ThemedText>
            <View style={[theme.layout.row, { gap: s.sm, marginBottom: s.xl, flexWrap: 'wrap' }]}>
              {['cash', 'upi', 'bank_transfer', 'cheque'].map(mode => (
                <TouchableOpacity 
                  key={mode}
                  onPress={() => setPaymentMode(mode)}
                  style={{ paddingHorizontal: s.md, paddingVertical: s.sm, backgroundColor: paymentMode === mode ? c.primary : c.surface, borderRadius: theme.borderRadius.sm, borderWidth: 1, borderColor: paymentMode === mode ? c.primary : c.border }}
                >
                  <ThemedText variant="caption" color={paymentMode === mode ? '#FFF' : c.onSurface} style={{ textTransform: 'capitalize' }}>
                    {mode.replace('_', ' ')}
                  </ThemedText>
                </TouchableOpacity>
              ))}
            ...
            <View style={{ padding: s.md, backgroundColor: parseFloat(amountPaid) >= grandTotal ? c.success + '20' : c.warning + '20', borderRadius: theme.borderRadius.md }}>
               <ThemedText weight="bold" color={parseFloat(amountPaid) >= grandTotal ? c.success : c.warning}>
