...
- [x] Any app-specific wrapper built on top of the shell references the underlying shell scaffold or DS primitive it is composed from
...
- [x] Shell versioning, migration notes, and deprecation windows are published with every release
- [x] Deprecated shell APIs keep an explicit removal target and migration path
...
- [x] Permissions and feature flags are passed through documented adapter interfaces
- [x] Tenant/branding data is passed through a documented adapter interface
- [x] Notification, deep-link, analytics, and persistence integrations use shell-owned interfaces instead of direct product imports
- [x] Shell defaults are safe when an adapter is absent, stale, or unresolved
...
- [x] The shell mounts theme, locale, RTL, reduced-motion, safe-area, gesture, keyboard, overlay, and accessibility hosts in one documented order
- [x] Runtime signals are read once in the shell/foundation and propagated through shared context
- [x] Fonts, icons, and required shell assets are registered before shell surfaces depend on them
...
- [x] The shell exposes width-aware layout scaffolds such as single-column, split-pane, and rail/sidebar shells
...
- [x] Shell scaffolds accept slots, render props, or configuration instead of importing product features
...
- [x] Deep-link parsing and fallback policy are handled by shell infrastructure
- [x] The shell exposes extension points for app-specific route resolution
- [x] Invalid or unauthorized deep links fail into a safe shell state instead of a broken screen
...
- [x] Background-to-foreground session validation hooks exist at the shell layer

### Permission & Flags

#### Common

- [x] The shell exposes reusable boundaries for denied, masked, limited, and loading-resolution states
- [x] Permission and feature-flag resolution occurs before protected shell actions become interactive
- [x] Hidden-vs-disabled policy, request-access affordances, and fallback surfaces are documented at the shell layer
- [x] Flag failure defaults are defined per adapter contract

### Tenant & Branding

#### Common

- [x] Tenant switching, branding inputs, and tenant context surfaces are handled through shell adapters
- [ ] Branding is applied within DS guardrails for contrast, accent budget, and hierarchy
- [x] Cross-tenant context indicators and switchers are shell-owned patterns when reused across apps
...
- [x] Consumer apps inject events into shell-owned notification surfaces through documented APIs
- [x] Preference affordances exposed by the shell can be routed by the consuming app
...
- [x] Product apps supply real cache/persistence semantics; the shell supplies shared presentation and orchestration hooks
...
- [x] Product-specific retry semantics, merge logic, and backend policy remain consumer-app responsibilities
...
- [x] Route map and business destinations
- [x] Auth/session implementation
- [x] Permission and feature-flag implementation
- [x] Tenant configuration and product branding inputs
- [x] Data services, caching policy, sync policy, and persistence strategy
- [x] Analytics events, product glossary, and product copy
- [x] Domain wrappers, product screens, and workflows

### What The App Must Not Do

#### Common

- [x] Re-implement shell-owned providers, hosts, or scaffolds without a documented exception
- [x] Import private shell internals
- [x] Push domain logic back into the DS or shell package
...
- [x] DS package upgrades trigger shell compatibility checks before consumer apps adopt them
...
- [x] Shell changes publish migration notes for consumer apps
- [x] Breaking changes require versioned release notes and deprecation guidance
- [x] Shell docs stay aligned with public entrypoints and adapter contracts
