...
jest.mock('react-native', () => {
  ...
  const ActivityIndicator = (props: any) => React.createElement('ActivityIndicator', { ...props, testID: 'ActivityIndicator' });
  ...
  const Appearance = { getColorScheme: jest.fn(() => 'light'), addChangeListener: jest.fn(() => ({ remove: jest.fn() })) };
  ...
  const Pressable = ({ children, ...props }: any) => React.createElement('Pressable', props, children);
  ...
  return {
    View, Text, ScrollView, TouchableOpacity, TextInput, RNTextInput: TextInput, ActivityIndicator, Platform, StyleSheet, Appearance, Alert, KeyboardAvoidingView, Touchable, Pressable, Modal, RefreshControl, FlatList, SectionList,
  };
});
