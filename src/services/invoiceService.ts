...
export const invoiceService = {
  ...
  async createInvoice(input: InvoiceInput): Promise<Invoice> {
    ...
    const { data: invoiceNumData, error: numError } = await supabase.rpc('generate_invoice_number');
    ...
    const finalItems = line_items.map((item, index) => {
      ...
      const tax = calculateLineItemTax(item.gst_rate, item.quantity, item.rate_per_unit, discount, input.is_inter_state);
