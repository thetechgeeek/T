  ...
  describe('createInvoice', () => {
    it('successfully saves invoice, links items, and deducts stock', async () => {
      ...
      (supabase.rpc as jest.Mock).mockImplementation((name) => {
        if (name === 'generate_invoice_number') return Promise.resolve({ data: mockInvoiceNum, error: null });
        if (name === 'perform_stock_operation') return Promise.resolve({ data: 10, error: null });
        return Promise.resolve({ data: null, error: null });
      });

      // Mock invoice insert
      mockQuery.then.mockImplementationOnce((resolve: any) => 
        resolve({ data: { id: mockInvoiceId, invoice_number: mockInvoiceNum }, error: null })
      );
      ...
      expect(supabase.rpc).toHaveBeenCalledWith('perform_stock_operation', expect.objectContaining({
        p_item_id: 'item-1',
