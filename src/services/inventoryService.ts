...
import type { InventoryItem, InventoryItemInsert, InventoryFilters, StockOpType, StockOperation } from '@/src/types/inventory';
...
export const inventoryService = {
  ...
  async fetchItems(filters: InventoryFilters, page = 1, pageSize = 20) {
    ...
    if (filters.search) {
      query = query.or(`design_name.ilike.%${filters.search}%,base_item_number.ilike.%${filters.search}%`);
    }
