...
describe('Screen', () => {
	...
	it('uses keyboard avoidance by default on the screen shell', () => {
		const { toJSON } = renderWithTheme(
			<Screen>
				<Text>Keyboard-aware body</Text>
			</Screen>,
		);

		const json = asSingleNode(toJSON());
		expect(json.props.behavior).toBe(Platform.OS === 'ios' ? 'padding' : undefined);
	});
	...
	it('can dismiss the keyboard on background tap when enabled', () => {
		...
		const { UNSAFE_getByType } = renderWithTheme(
			<Screen dismissKeyboardOnBackgroundTap withKeyboard={false}>
				<Text>Body</Text>
			</Screen>,
		);
