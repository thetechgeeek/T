...
describe('SwipeableRow', () => {
	...
	it('supports archive actions and screen-reader accessibility actions', () => {
		const onArchive = jest.fn();
		const onDelete = jest.fn();
		const { getByTestId } = renderWithTheme(
			<SwipeableRow testID="swipe-row" onDelete={onDelete} onArchive={onArchive}>
				<Text>Item</Text>
			</SwipeableRow>,
		);

		fireEvent.press(getByTestId('swipeable-archive-btn'));
		expect(onArchive).toHaveBeenCalledTimes(1);

		fireEvent(getByTestId('swipe-row'), 'accessibilityAction', {
