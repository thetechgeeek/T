...
describe('inventoryStore', () => {
  ...
  it('performStockOperation updates the local state correctly after a successful API call', async () => {
    ...
    (inventoryService.fetchItemById as jest.Mock).mockResolvedValue({ ...mockItems[0], box_count: 70 });

    await useInventoryStore.getState().performStockOperation('1', 'stock_in', 20, 'Restock');

    const state = useInventoryStore.getState();
    expect(inventoryService.performStockOperation).toHaveBeenCalledWith('1', 'stock_in', 20, 'Restock');
