...
import type { InventoryItem, InventoryFilters, StockOpType, TileCategory } from '../types/inventory';
...
interface InventoryState {
  ...
  performStockOperation: (itemId: UUID, type: StockOpType, qty: number, reason?: string) => Promise<void>;
  reset: () => void;
}
...
export const useInventoryStore = create<InventoryState>()(
  immer((set, get) => ({
    ...
    fetchItems: async (reset = false) => {
      ...
      try {
        const { data, count } = await inventoryService.fetchItems(state.filters, pageToFetch, PAGE_SIZE);
