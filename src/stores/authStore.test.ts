...
describe('authStore', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    useAuthStore.setState({ user: null, session: null, loading: false, isAuthenticated: false });
  });

  it('initialize sets state from session', async () => {
    const mockSession = { user: { id: '1' } };
    (authService.getSession as jest.Mock).mockResolvedValue(mockSession);
    (authService.onAuthStateChange as jest.Mock).mockReturnValue({ data: { subscription: { unsubscribe: jest.fn() } } });
  ...
  it('logout resets the state', async () => {
    useAuthStore.setState({ user: { id: '1' } as any, session: {} as any, isAuthenticated: true });
