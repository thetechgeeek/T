...
const legacyGeneratedRoot = path.join(root, 'src', 'theme', 'generated');
...
function resolveGeneratedMirrorTargets(targetPath) {
	if (!targetPath.startsWith(generatedRoot)) {
		return [targetPath];
	}

	const relativePath = path.relative(generatedRoot, targetPath);
	return [targetPath, path.join(legacyGeneratedRoot, relativePath)];
}

function writeMirroredGeneratedFile(targetPath, contents) {
	for (const outputPath of resolveGeneratedMirrorTargets(targetPath)) {
...
writeMirroredGeneratedFile(jsonOutputPath, `${JSON.stringify(w3cPayload, null, 2)}\n`);
...
writeMirroredGeneratedFile(cssOutputPath, `${cssLines.join('\n')}\n`);
...
writeMirroredGeneratedFile(scssOutputPath, `${scssLines.join('\n')}\n`);
...
writeMirroredGeneratedFile(androidOutputPath, androidXml.join('\n'));
...
writeMirroredGeneratedFile(iosSwiftOutputPath, iosSwift.join('\n'));

for (const outputPath of resolveGeneratedMirrorTargets(iosAssetCatalogPath)) {
	...
	for (const entry of androidColorEntries) {
		...
		writeIosColorAsset(path.join(outputPath, assetName), entry.value);
	}
