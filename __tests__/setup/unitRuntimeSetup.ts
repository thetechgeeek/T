 ...
 *
 * SUPABASE: Unit tests receive a default mock so they stay env-free. Tests that
 * assert Supabase behavior should override it locally with:
 *   jest.mock('../config/supabase', () => ({ supabase: createSupabaseMock() }))
 * Integration tests do not load this setup file.
 *
...
// Default unit Supabase mock. Integration tests use jest.integration.config.js instead.
...
// Mock i18next globally. Translation JSON is loaded lazily so pure unit tests
// that never translate do not load app locale data at setup time.
jest.mock('i18next', () => {
	...
	const t = (key: string, opts?: Record<string, unknown>): string => {
		...
		let val = allTranslations[key];
		if (!val) {
			// Fallback: last segment of key
			const shortKey = key.split('.').pop() || key;
			val = allTranslations[shortKey] || (opts?.defaultValue as string) || shortKey;
		}
...
// NOTE: This file is the unit-test runtime only. Integration tests use
// jest.integration.config.js so real Supabase configuration can fail fast.
